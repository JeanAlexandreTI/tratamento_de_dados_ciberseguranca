...
As variáveis de ambiente são carregadas a partir de um arquivo .env, contendo credenciais do banco de dados como host, usuário, senha e nome do banco. Isso evita subir senha junto com o projeto.
...
Depois disso, o script entra na fase de padronização textual. Códigos de ticket, severidade e status são convertidos para letras maiúsculas. Tipos de incidentes passam por correções ortográficas clássicas, como brute-fore virando brute-force e pishing sendo corrigido para phishing.

Registros duplicados são removidos com base no ticket_code, mantendo apenas uma ocorrência por ticket, e os dados são ordenados pelo incident_id.

# Resultado final

Ao final da execução, o script imprime no terminal um DataFrame limpo, padronizado, sem duplicidades óbvias e com regras de negócio aplicadas. Esse dataset já está pronto para ser usado em análises, dashboards ou exportações futuras.
...
Esse arquivo não deve ser versionado. Coloque-o no .gitignore.

# Observações finais

Este script não foi feito para ser genérico ou bonito, foi feito para ser realista. Ele reflete exatamente o tipo de tratamento que acontece quando dados estão incorretos e sem padrão em ambientes corporativos.
